...
impl Resource {
    pub fn can_fulfill<'a>(&self, doc: &'a Document) -> Vec<&'a Resource> {
        let node_id = *doc.resources_map.get(&self.name).unwrap();
        ...
        for neighbor_id in neighbor_ids {
            ...
            let neighbor = doc.resources.node_weight(neighbor_id).unwrap();
