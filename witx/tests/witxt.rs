...
impl WitxtRunner<'_> {
    ...
    fn test_directive(
        ...
        match directive {
            ...
            | WitxtDirective::AssertRepresentable { repr, t1, t2, .. } => {
                ...
                let t1 = t1d
                    .typename(&wazzi_witx::Id::new(t1t))
    ...
    fn assert_md(&self, doc: &wazzi_witx::Document) -> Result<()> {
        self.bump_ntests();
        doc.to_md();
        Ok(())
    }
...
impl wazzi_witx::Bindgen for AbiBindgen<'_> {
    type Operand = ();
    fn emit(
        &mut self,
        inst: &Instruction<'_>,
    ...
    fn allocate_space(&mut self, _: usize, _: &wazzi_witx::NamedType) {
...
enum WitxDef<'a> {
    Fs(Vec<&'a str>),
    Inline(Vec<wazzi_witx::parser::Documented<'a, wazzi_witx::parser::DeclSyntax<'a>>>),
}
